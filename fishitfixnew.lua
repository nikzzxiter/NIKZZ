...
local function SafeRespawn()
	...
	if SavedPosition then
		...
		warn("[Anti-Stuck] ✅ Teleport restored.")
	else
		warn("[Anti-Stuck] ⚠️ No saved position.")
	end
...
function AutoFishingV1()
	...
	task.spawn(function()
		...
		while Config.AutoFishingV1 and not IsRespawning do
			if CycleLocked then task.wait(0.02) continue end
			CycleLocked = true
